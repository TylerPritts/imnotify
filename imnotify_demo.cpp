...
namespace ImNotify
{
    static void ShowDemoWindow(bool* p_open = NULL)
    {
		...
		if (ImGui::CollapsingHeader("Examples without title", ImGuiTreeNodeFlags_DefaultOpen))
		{
			...
			{
				ImNotify::InsertNotification({ ImGuiToastType_Warning, 3000, "Hello World! This is a warning!" });
			}

			ImGui::SameLine();
			if (ImGui::Button("Error"))
			{
				ImNotify::InsertNotification({ ImGuiToastType_Error, 3000, "Hello World! This is an error!" });
			}
